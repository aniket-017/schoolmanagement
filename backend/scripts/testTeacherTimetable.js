  ...
  try {
    ...
    timetables.forEach((timetable, index) => {
      ...
      timetable.periods.forEach((period, pIndex) => {
        console.log(`     ${pIndex + 1}. Subject: ${period.subject?.name || 'Unknown'}, Teacher: ${period.teacher?.name || 'Unknown'}, Time: ${period.startTime}-${period.endTime}`);
      });
    });
    ...
    if (teachers.length > 0) {
      ...
      if (teacherTimetables.length > 0) {
        teacherTimetables.forEach((timetable, index) => {
          console.log(`\nTimetable ${index + 1}: Day: ${timetable.day}, Class: ${timetable.classId?.grade}${timetable.classId?.division}`);
          ...
          teacherPeriods.forEach((period, pIndex) => {
            console.log(`  ${pIndex + 1}. ${period.subject?.name || 'Unknown'} (${period.startTime}-${period.endTime})`);
          });
        });
      }
    }
