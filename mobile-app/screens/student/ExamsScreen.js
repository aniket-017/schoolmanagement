...
export default function ExamsScreen({ navigation }) {
  ...
  const loadExams = async () => {
    try {
      ...
      if (!classId) {
        console.log("No class ID found for user:", user);
        setExams([]);
        setGroupedExams([]);
        return;
      }

      console.log("Loading exams for class ID:", classId);
      
      // Always load individual exams and group them by name automatically
      const response = await apiService.examinations.getExaminationsByClass(classId, {
        upcoming: "true",
        limit: 50,
      });
      
      console.log("Individual exams response:", response);
      ...
      if (response && response.success && Array.isArray(response.data)) {
        ...
        console.log("Grouped exams:", grouped);
        ...
        {loading ? (
          ...
          <View style={styles.emptyContainer}>
            ...
            <Text style={styles.emptySubtext}>
              {filter === "all" 
                ? "Your examinations will appear here once scheduled by your teachers"
